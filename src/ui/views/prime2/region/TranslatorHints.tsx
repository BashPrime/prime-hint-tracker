...
import { PRIME_2_ALL_ITEMS_VALUES, PRIME_2_LOCATIONS_WITH_ITEMS, PROXIMITY_OPTIONS } from "@/data/Prime2.data";
        ...
        <div key={`trans-hint-${idx}`}>
          ...
          <div className="flex flex-col">
            ...
            <div className="flex flex-row">
              <AutoComplete placeholder="Proximity" emptyMessage="No matching option found." options={proximityOptions} className="bg-zinc-700 w-16 h-6" />
              {translatorHint.proximityType !== "in" && <Input placeholder="# rooms" type="number" value={translatorHint.numRooms} className="bg-slate-700 w-14 h-6" />}
            </div>
            {translatorHint.proximityType === "in" && <AutoComplete placeholder="Location..." emptyMessage="No location found." options={locationOptions} className="text-sm h-6" />}
            {translatorHint.proximityType !== "in" && (
              <div className="flex flex-col items-start gap-1">
                <p className="text-sm text-zinc-400">from</p>
                <AutoComplete placeholder="????" emptyMessage="No location found." options={secondValueOptions} className="text-sm h-6" />
              </div>
            )}
          </div>
        </div>
